...
var idScope = '0ne003ACD87';
var registrationId = 'IceCreamRegulator2';
var symmetricKey = 'co32zq5Zxx/Kt1R3BSKvCmfMKN5YLLuEF1gjPVxlsec=';

var provisioningSecurityClient = new SymmetricKeySecurityClient(registrationId, symmetricKey);
var provisioningClient = ProvisioningDeviceClient.create(provisioningHost, idScope, new ProvisioningTransport(), provisioningSecurityClient);
var hubClient;

var truckIdentification = "Truck number 2";

var rest = require("azure-maps-rest");

// Enter your Azure Maps key
var subscriptionKeyCredential = new rest.SubscriptionKeyCredential("nc6FarKtWQT2DTiGsQkxGLsBPXgv-zIDL2i_55nQqfU");
...
var stateEnum = Object.freeze({ "ready": "ready", "enroute": "enroute", "delivering": "delivering", "returning": "returning", "loading": "loading", "dumping": "dumping", "parked": "parked" });
...
var truckSectionsCompletedTime = 0; // The time the truck has spent on previous completed sections. 

// EXTENSION: new value incl to capure driver's state and related data
var driverStateEnum = Object.freeze({"energized" : "energized", "active" : "active", "tired" : "tired", "fatigued" : "fatigued", "resting": "resting"});
var currentDriverState = driverStateEnum.energized;
var enrouteDuration = 0;
var restInverval;

const restActiveInterval = 2*60;
const restTiredInterval = 2 * restActiveInterval;
const restFatiguedInterval = 1.5 * restTiredInterval;
...
function UpdatePosition() {
    while ((truckSectionsCompletedTime + timeOnPath[truckOnSection] < timeOnCurrentTask) && (truckOnSection < timeOnPath.length - 1)) {
        ...
        updateDriverState(timeOnPath[truckOnSection]);
...
function updateDriverState(timeOnPath){

    if (state == stateEnum.enroute)
    {
        enrouteDuration = enrouteDuration + timeOnPath;

    }      
    else if (state == stateEnum.returning && enrouteDuration > 0)
    {
        enrouteDuration = enrouteDuration - timeOnPath;
    }

    if (enrouteDuration > restActiveInterval){
        currentDriverState = driverStateEnum.active;
    }
     if (enrouteDuration > restTiredInterval){
        currentDriverState = driverStateEnum.tired;
    }
    if (enrouteDuration > restFatiguedInterval){
        currentDriverState = driverStateEnum.fatigued;
    }


}

// method to register commands for driver to take rest break
function CmdDriverCooldown(request, response) {

    // GIVEN IN MINUTES        
        if (currentDriverState
            == driverStateEnum.fatigued)
        {
            state = stateEnum.parked;
            currentDriverState = driverStateEnum.resting;
            restInverval = setInterval(FncRestDriver, 5000);
        }

    response.send(200, 'Success', function (errorMessage) {

        if (errorMessage) {
            redMessage('Failed sending a CmdDriverRest response:\n' 
            + errorMessage.message);
        }
    });
}
// separate function to process data in intervals
function FncRestDriver(){

    enrouteDuration -= enrouteDuration/2;
    if (enrouteDuration < 1)
    {
        currentDriverState = driverStateEnum.energized;
        clearInterval(restInverval);
        GetRoute(stateEnum.enroute);
    }
    ...
    timeOnCurrentTask += interval;
        ...
        switch (state) 
        {
            ...
            case stateEnum.enroute:
                ...
                // new addition: truck won't move until driver fully rested again (energized)
                if (currentDriverState != driverStateEnum.fatigued)
                {
                    ...
                    console.log("STUCK! MUST REST DRIVER");
                }
                break;
            case stateEnum.dumping:
...
function sendTruckTelemetry() {
    ...
    // Simulate the truck. 
    UpdateTruck();
    var temperatureTemp = temp.toFixed(2);
    ...
    var data = JSON.stringify(
        {
            ...
            ContentsTemperature: temperatureTemp,
            TruckState: state,
            CoolingSystemState: fan,
            ContentsState: contents,
            // write JSON field to send drivers current state,
            // total duration of both their travel, and rest during travel to IoT Central device
            DriverState: currentDriverState,
            EnrouteTotal: truckSectionsCompletedTime,
            RestTotal: enrouteDuration,
            Location: {
    ...
    if (err) {
        ...
        hubClient.getTwin((err, twin) => {
            if (err) {
                ...
                // added method to register commands for driver to take rest break
                hubClient.onDeviceMethod('RestDriver', CmdDriverCooldown);
            }
        });
    }
};
...
provisioningClient.register((err, result) => {
    if (err) {
        ...
        // var connectionString = 'HostName=iotc-0ne003ACD87.azure-devices.net' 
        //     + result.assignedHub + ';DeviceId=IceCreamRegulator1' 
        //     + result.deviceId + ';SharedAccessKey=P2u7cX2hMjRQCcxs9INYrcD2f7VddZX7KrZt2de0/gI=' + symmetricKey;
